...
######################## description ###############################
# 
# 1. add IADDL and ILEAVE instruction
#	 
#	just like PartB seq
#
# 2. optimize 
...
#				ret
#
#    reduce ret instuction's 3 bubble to 1 bubble
#
# 3. optimize
#				mrmovl
#				opl (andl)
#
#    solve the load/use hazard by the simulator's bug about sequence run in a circle, 
#    cause the two instruction no bubble or stall.
#	
# 4. optimize
#				opl (andl)
#				mrmovl
#				xxxx (not opl)
#				jle
#	 mrmovl use opl's setCC calculate the Cond, and send Cond to jle's fetch, make jle
#    predict 100% success.
#
#
# All those optimizes are just matching my ncopy.ys 's instruction sequence
#
# those modifiction in this .hcl marked " Azard's xxx"
# Azard is my (熊伟伦) English name
#
# get some inspiration from Internet and tcbbd
# but the code is worked by myself
# it's impossible for you to find someone's code same as me
# maybe the optimize 4 ---- op mrmovl xxx jle , this optimize thinking a little same
# but realization is truely different, my code not include any new value and seem more
# comfortable.
#
# This lab is very hard
#														by  Azard
#                                           When I get 7.69 point
#											 at 14:51  2013/10/13
#
####################################################################
...
int f_pc = [
 ...
 && !(E_icode in {IOPL,IIADDL}) && M_icode == IMRMOVL
 && !M_Cnd : D_valP;
	# end
...
int f_ifun = [
	imem_error : FNONE;
	# Azard's jle
	f_icode == IMRMOVL : 1;  # jle's Cond calculas
	# end
	1: imem_ifun;
];
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd &&
	 !(E_ifun == 1 && W_icode == IMRMOVL && !(M_icode in {IOPL, IIADDL}))) ||
	# Azard's jle

	# Stalling at fetch while ret passes through pipeline
	# but not condition for a load/use hazard
	!(E_icode in { IMRMOVL, IPOPL, ILEAVE } && E_dstM in { d_srcA, d_srcB }
			&& !(E_icode == IMRMOVL && D_icode == IOPL)  # Azard's rm andl
	) &&
	# Azard's ret
	(  
		D_icode == IRET  && E_icode != IPUSHL ||
